...
2. **[Input]** Trace each material contract rule to the design, implementation, and verification coverage, looking specifically for omissions, invented behavior, altered errors/edge cases, and data-integrity drift.
3. **[Input]** Challenge target structure for accidental WPF/MSSQL carryover, platform/DLL coupling leakage, unsupported assumptions, and scope added beyond the approved design.
4. **[Output]** Write findings for `review.md` with severity, affected behavior/path, evidence, expected correction, and whether the finding blocks verification or completion.
