...
4. [Input] Run the judge against the declared legacy/contract and target observations; validate the judge with a controlled mismatch under the mandatory negative-control self-check gate in `docs/03-evidence-and-verification.md`, and record its status and evidence using `docs/templates/verification.md`.
