...
- Issue #9 now has a dedicated design in `docs/09-evidence-grade-transition-control.md`; issue #6 must reference that stricter grade-transition contract rather than redefining it.
- Issue #10 added provenance rules to several skills, but those edits still do not provide a uniform `Inputs`/`Outputs`/`Branches` execution contract. Issue #6 must preserve the provenance rules while normalizing execution structure.
...
9. Issue #9 (evidence-grade history), issue #10 (observed/inferred provenance), issue #11 (verification-judge self-check), and issue #7 (routing) are stricter or orthogonal controls. Issue #6 must preserve them rather than flattening them into generic branching prose.
...
Before a skill procedure starts, its caller resolves the scope using `docs/09-agent-skill-routing.md`.
...
9. preserve `docs/09-agent-skill-routing.md` ownership/tie-break rules; this execution contract must not redefine routing triggers.
...
- routing ownership from issue #7 remains intact;
