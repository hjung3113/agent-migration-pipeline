...
| S-011 | DONE | 복잡 | 하 | Pipeline dry-run with synthetic feature + mutation self-test | migration/features/synthetic-demo/, migration/judge/tests/ (mutation self-test PASS — judge catches injected mismatch), 1 process fix (judge test PYTHONPATH). Lock-in low, no Opus review per policy |
